...
- **Unknown members** (`-1`) in every dimension, with `COALESCE`/`CASE` logic in the fact loads so no fact row ever holds a NULL foreign key, enforcing referential integrity.
